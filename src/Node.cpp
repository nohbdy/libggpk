...
enum NodeTypeId : int32_t {
	FILE = 0x454C4946,	// 'FILE'
	PDIR = 0x52494450,	// 'PDIR'
	FREE = 0x45455246	// 'FREE'
};
...
inline std::string read_node_name(QFile* file, int name_length) {
	std::string result;
	char* name_data = new char[2 * name_length];

	file->read(name_data, 2 * name_length);

	result = ToNarrow((wchar_t*)name_data);
